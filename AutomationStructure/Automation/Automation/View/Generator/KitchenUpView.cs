...
namespace Automation.View
{
    ...
    public class KitchenUpView : IViewGenerator
    {
        ...
        public void SetupView(RadGridView dgv, DataTable table)
        {
            dgv.Columns.Clear();
            dgv.DataSource = null;
            resultTable = table;
        ...
        private void Dgv_CellClick(object sender, GridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 3)
            {
                var path = resultTable.Rows[e.RowIndex][2].ToString().Replace("icon.png","big.png");

                Form customerHelpForm = Application.OpenForms["BigModuleImageInfo"];
                if (customerHelpForm == null)
                {
                    customerHelpForm = new BigModuleImageInfo(path);
        ...
        private ColumnGroupsViewDefinition SetColumnGroupsView(RadGridView dgv)
        {
            ...
            view.ColumnGroups.Add(new GridViewColumnGroup("Фасад","facades"));
        ...
        private void Dgv_CellFormatting(object sender, CellFormattingEventArgs e)
        {
            try
            {
                if (e.CellElement.ColumnIndex == 3)
                {
                    ...
                    {
                        var pathToImage = resultTable.Rows[e.RowIndex]["Изображение"].ToString();
